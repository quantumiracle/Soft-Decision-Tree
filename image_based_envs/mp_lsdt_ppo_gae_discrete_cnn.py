...
from mnist_test.landmarkSDT import SDT
...
class PPO(nn.Module):
    def __init__(self, obs_space, action_space, learner_args, hidden_dim=128):
        ...
        self.fc_h = nn.Linear(in_layer_dim, hidden_dim)
        self.fc_v  = nn.Linear(hidden_dim,1)
        self.dt = SDT(learner_args)

        self.optimizer = SharedAdam(list(self.parameters())+list(self.dt.parameters()), lr=learning_rate)


    def pi(self, x):
        return self.dt.forward(x, LogProb=False)[1]
...
if __name__ == '__main__':
    ...
    learner_args = {'input_shape': env.observation_space.shape,  # CHW
