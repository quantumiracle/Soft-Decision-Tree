...
def draw_tree(tree, input_shape, input_img=None, show_correlation=False, savepath='', Landmarks=True):
    ...
    inner_nodes = tree.state_dict()['conv_kernels']
    ...
    prediction,_,_,_ = tree.forward(x)
    pred = prediction.data.max(1)[1]  # prediction result
    print(pred)
    feature_maps = tree.feature_maps.squeeze()
    landmarks = tree.landmarks

    kernels = dict([(node_idx, node_value.detach().cpu().numpy().reshape((img_rows, img_cols))) for node_idx, node_value in zip (inner_indices, feature_maps) ])
    landmarks = dict([(node_idx, node_value.detach().cpu().numpy()) for node_idx, node_value in zip (inner_indices, landmarks) ])
    ...
    for pos, key in enumerate(sorted(kernels.keys(), key=lambda x:(len(x), x))):
        ...
        if len(kernel_image.shape)==3 or len(kernel_image.shape)==2: # 2D image (H, W, C)
            ax.imshow(kernel_image.squeeze(), vmin=kernel_min_val, vmax=kernel_max_val, aspect='equal', **imshow_args)
            if Landmarks:  # not work right yet
                from matplotlib.patches import Circle
                x, y = landmarks[key]
                ax.add_patch(Circle((img_rows/2*(x+1), img_cols/2*(y+1)), radius=1, color='green'))
    ...
    if input_img is not None:
        ...
        if len(input_img.shape)==3 or len(input_img.shape)==2: # 2D image (H, W, C)
        ...
        # distinguish with solid or dotted lines
        _add_arrow(ax, axes['0'],
                   (1, img_rows//2), (img_cols-1, img_rows//2), 'black', None)
