...
class SDT(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.input_dim = self.input_y_dim*self.input_x_dim
        ...
        self.weights = nn.Parameter(torch.randn(self.inner_node_num, self.input_dim))
        self.target = nn.Parameter(torch.randn(self.inner_node_num, self.input_dim))
    ...
    def inner_nodes(self, x):
        # masked_input=torch.einsum('ij,kj->ikj', x, self.sigmoid(self.mask))  # (batch, input_dim), (self.inner_node_num, input_dim) -> (batch, self.inner_node_num, input_dim)
        # masked_input=torch.einsum('ij,kj->ikj', x, self.mask)  # (batch, input_dim), (self.inner_node_num, input_dim) -> (batch, self.inner_node_num, input_dim)
        # self.inner_probs = self.sigmoid(self._imgs_diff(masked_input, self.target)+self.bias)  # (batch, self.inner_node_num)
        
        self.product = torch.einsum('ij,kj->ik', x, self.target) # (batch, input_dim), (self.inner_node_num, input_dim) -> (batch, self.inner_node_num)
        self.inner_probs = self.sigmoid(self.product+self.bias)
        # self.inner_probs = self.sigmoid(self.bias+torch.sum(masked_input, dim=-1))
    ...
    def get_tree_weights(self, Bias=False):
        ...
        if Bias:
            return self.state_dict()['weights'].detach().cpu().numpy(), self.state_dict()['target'].detach().cpu().numpy(), self.state_dict()['bias'].detach().cpu().numpy()
        else:  # no bias
            return self.state_dict()['weights'].detach().cpu().numpy(), self.state_dict()['target'].detach().cpu().numpy()

    def _imgs_diff(self, img1, img2, scale=255.):
        loss = nn.MSELoss(reduction='none') 
        diff = loss(img1, img2)   # (batch, self.inner_node_num, input_dim)
        mean_diff = diff.mean(-1)/(scale**2)  # (batch, self.inner_node_num)
        return mean_diff  # set of differences
