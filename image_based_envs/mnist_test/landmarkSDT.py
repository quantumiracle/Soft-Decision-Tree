...
class SpatialSoftmax(torch.nn.Module):
    ...
    def forward(self, feature):
        ...
        return feature_keypoints  # to plot with scatter, the first dim is x, the second dim y
...
class SDT(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.input_channel, self.input_y_dim, self.input_x_dim=self.args['input_shape'] # image width
        ...
        self.weights = nn.Parameter(torch.randn(self.inner_node_num, 2))
        self.bias = nn.Parameter(torch.randn(self.inner_node_num))
        KernelSize = 5 # odd, so padding can ensure feature maps to have same size as original
        self.Padding = int((KernelSize-1)/2)  # no size change after convolution
        self.conv_kernels = nn.Parameter(torch.randn(self.inner_node_num, 1, KernelSize, KernelSize).repeat(1, self.input_channel, 1,1))
        self.ssm = SpatialSoftmax(self.input_y_dim,self.input_x_dim,self.input_channel,temperature=1.)
    ...
    def inner_nodes(self, x):
        self.feature_maps = F.conv2d(x, self.conv_kernels, padding=self.Padding)  # (batch, self.inner_node_num, self.input_x_dim, self.input_y_dim)
        self.landmarks = self.ssm(self.feature_maps.view(self.batch_size*self.inner_node_num, 1, self.input_x_dim, self.input_y_dim)) # (batch*self.inner_node_num, 2)
        logits=torch.einsum('bij,ij->bi', self.landmarks.view(self.batch_size, self.inner_node_num, -1), self.weights)+self.bias # (batch, self.inner_node_num)
        # self.inner_probs = self.sigmoid(logits)  # (batch, self.inner_node_num)
        self.inner_probs = self.sigmoid(self.beta*logits)  # (batch, self.inner_node_num)
        return self.inner_probs

    def get_tree_weights(self, Bias=False):
        """Return tree weights as a list"""
        if Bias:
            return self.state_dict()['conv_kernels'].detach().cpu().numpy(), self.state_dict()['weights'].detach().cpu().numpy(), self.state_dict()['bias'].detach().cpu().numpy()
        else:  # no bias
            return self.state_dict()['conv_kernels'].detach().cpu().numpy(), self.state_dict()['weights'].detach().cpu().numpy()
    ...
    def _forward(self, data):
        ...
        _mu = data.data.new(self.batch_size,1,1).fill_(1.)
        ...
        mu = _mu.view(self.batch_size, self.leaf_num)  
    ...
    def _cal_penalty(self, layer_idx, _mu, _path_prob):
        ...
        _mu = _mu.view(self.batch_size, 2**layer_idx)
        _path_prob = _path_prob.view(self.batch_size, 2**(layer_idx+1))
        alpha_list=[]
    ...
    def _data_augment_(self, input):
        ...
        input = input.view(self.batch_size, 1, input.size()[-2], input.size()[-1])  # for Minist
        return input
