...
class SDT(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.mask = nn.Parameter(torch.randn(self.inner_node_num, self.args['input_dim']))
        self.target = nn.Parameter(torch.randn(self.inner_node_num, self.args['input_dim']))
    ...
    def get_tree_weights(self, Bias=False):
        ...
        if Bias:
            return self.state_dict()['mask'].detach().cpu().numpy(), self.state_dict()['target'].detach().cpu().numpy(), self.state_dict()['bias'].detach().cpu().numpy()
        else:  # no bias
            return self.state_dict()['mask'].detach().cpu().numpy(), self.state_dict()['target'].detach().cpu().numpy()
