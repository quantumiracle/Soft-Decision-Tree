...
class PPO(nn.Module):
    def __init__(self, obs_space, action_space, hidden_dim=256):
        ...
        self.fc_pi = nn.Linear(in_layer_dim,self.action_dim)  
        self.fc_v  = nn.Linear(in_layer_dim,1)
...
def run(env, train=False, test=False):
    model = PPO(env.observation_space, env.action_space)
    episode_r_list=[]
    episode_r = 0.0
    print_interval = 10
    model_path = './model/ppo_discrete'
    if test:
    ...
    for n_epi in range(TRAIN_EPI):
        ...
        if n_epi%print_interval==0 and n_epi!=0:
            if train:
                ...
                np.save('learn', episode_r_list)
            print("Number of episode :{} | Episode reward : {:.3f} | Episode length: {}".format(n_epi, episode_r, t))
...
def plot(rewards):
    ...
    plt.plot(rewards)
    plt.savefig('minatar.png')
    # plt.show()
    plt.clf()

if __name__ == '__main__':
    # env = gym.make('LunarLander-v2')
    env = GymWrapper(Environment(args.game))
    if args.train:
        run(env, train=True, test=False)
    if args.test:
        run(env, train=False, test=True)
