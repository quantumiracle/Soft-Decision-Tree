...
def train_tree(tree):
    writer = SummaryWriter(log_dir='runs_ppo_prob/'+'sdt_'+str(learner_args['depth'])+'_id'+str(args.id))
    ...
    criterion  = nn.KLDivLoss()  # kl-divergence of two probability distributions

    # Load data
    data_dir = './data/LunarLander-v2_ppo_'
    data_path = data_dir+'state.npy'
    label_path = data_dir+'prob.npy'
    ...
    for epoch in range(1, learner_args['epochs']+1):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            prediction, output, penalty, weights = tree.forward(data, LogProb=False)
            ...
            if batch_idx % learner_args['log_interval'] == 0:
                with torch.no_grad():
                    ...
                    print('Epoch: {:02d} | Batch: {:03d} | CrossEntropy-loss: {:.5f} | Difference: {}'.format(
                            epoch, batch_idx, loss.data, output.size()[0], difference))
        ...
        # print('\nEpoch: {:02d} | Testing Accuracy: {}/{} ({:.3f}%) | Historical Best: {:.3f}% \n'.format(epoch, len(test_loader.dataset), accuracy, best_testing_acc))
